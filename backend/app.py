...
# Ruta para apagar el servidor
@app.route('/shutdown', methods=['POST'])
def shutdown():
    """
    Endpoint para apagar el servidor de forma controlada
    Este endpoint es utilizado por el script terminate_app.py
    """
    logger.info("Recibida solicitud para apagar el servidor")
    
    # Esta función permitirá apagar correctamente el servidor Flask
    func = request.environ.get('werkzeug.server.shutdown')
    if func is None:
        # Si no estamos usando el servidor de desarrollo de Werkzeug
        # intentar otras formas de apagar
        try:
            # Para versiones modernas de Flask
            from werkzeug.serving import shutdown_server
            shutdown_server(request.environ)
            return jsonify({"success": True, "message": "Servidor apagado correctamente"})
        except ImportError:
            # Si no podemos importar, intentar con signal
            import signal
            import os
            try:
                # Enviar señal para apagar
                pid = os.getpid()
                os.kill(pid, signal.SIGTERM)
                return jsonify({"success": True, "message": "Señal de apagado enviada"})
            except Exception as e:
                logger.error(f"Error al intentar apagar el servidor: {str(e)}")
                return jsonify({"success": False, "error": str(e)}), 500
    else:
        # Usando Werkzeug
        func()
        logger.info("Servidor apagado por solicitud")
        return jsonify({"success": True, "message": "Servidor apagado correctamente"})
